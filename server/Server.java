...
public class Server {
    ...
    public Server(int port) {
        ...
        try (ServerSocket serverSocket = new ServerSocket(port)) {
            ...
            while (true) {
                log.trace("Ждем нового клиента..");
                Socket socket = serverSocket.accept();
                log.trace("Клиент подключился. Сервер : " + this + "socket : " + socket);
            ...
            log.debug("По блоку finally выполнен аварийный останов this.authenticationProvider.shutdown() ");
        }
    }
    ...
    public synchronized void subscribe(ClientHandler clientHandler) {
        ...
        log.trace("Клиент " + clientHandler.getUsername() + " вошел в чат");
    ...
    public synchronized void unsubscribe(ClientHandler clientHandler) {
        ...
        log.trace("Клиент " + clientHandler.getUsername() + " вышел из чата");
        broadcastClientsList();
    }
    ...
    public synchronized void sendPrivateMessage(ClientHandler sender, String receiverUsername, String message) {
        for (ClientHandler c : clients) {
            if (c.getUsername().equals(receiverUsername)) {
                ...
                log.trace("Сервер обработал приватное сообщение От: " + sender.getUsername() + "Пользователю: " + receiverUsername + " Сообщение: " + message);
                return;
            }
        }
        sender.sendMessage("Невозможно отправить сообщение пользователю: " + receiverUsername + ". Такого пользователя нет в сети.");
        log.trace("Невозможно отправить сообщение <" + message + "> пользователю: " + receiverUsername + ". Такого пользователя нет в сети.");
    }
